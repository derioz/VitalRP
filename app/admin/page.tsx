...
import { Users, Image, Activity, Shield, TrendingUp, Clock, ArrowRight, UserPlus } from 'lucide-react';
...
const Dashboard: React.FC = () => {
    ...
    useEffect(() => {
        // Fetch Live Server Stats via server endpoint
    ...
    const quickActions = [
        ...
        { label: 'Review Users & Roles', icon: Shield, path: '/admin/users' },
    ];
            ...
            >
                {statCards.map((stat, idx) => (
                    ...
                    >
                        ...
                        {stat.path && (
                            <Link href={stat.path} className="absolute inset-0 z-10" aria-label={`Go to ${stat.label}`} />
                        )}
                    </motion.div>
                ))}
            </motion.div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                >
                    <div className="flex justify-between items-center mb-6">
                        ...
                        <Link href="/admin/users" className="text-sm text-vital-500 hover:text-vital-400 font-medium flex items-center gap-1">
                    ...
                    <div className="space-y-4">
                        {loading ? (
                            ...
                            recentUsers.map((userItem, i) => (
                                <div key={userItem.id || i} className="flex items-center justify-between p-3 rounded-xl bg-dark-950/50 border border-white/5 hover:bg-white/5 transition-colors">
                                    <div className="flex items-center gap-3">
                                        <div className="w-10 h-10 rounded-full bg-gradient-to-br from-gray-700 to-gray-800 flex items-center justify-center text-white font-bold text-sm border border-white/10 overflow-hidden">
                                            {userItem.photoURL || userItem.avatar ? (
                                                <img src={userItem.photoURL || userItem.avatar} alt={userItem.displayName} className="w-full h-full object-cover" />
                                            ) : (
                                                (userItem.displayName || userItem.username || '?')[0].toUpperCase()
                                            )}
                                        </div>
                                        <div>
                                            <h4 className="text-white font-medium text-sm">{userItem.displayName || userItem.username || 'Unknown User'}</h4>
                                            <p className="text-xs text-gray-500 font-mono">{userItem.email || userItem.id}</p>
                                        </div>
                                    </div>
                                    <div className="text-right">
                                        <span className={`text-xs px-2 py-1 rounded-full border capitalize ${
                                            ['owner', 'management'].includes(userItem.role) ? 'bg-amber-500/10 text-amber-400 border-amber-500/20' :
                                            ['senior_admin', 'admin'].includes(userItem.role) ? 'bg-red-500/10 text-red-400 border-red-500/20' :
                                            ['moderator', 'support', 'staff'].includes(userItem.role) ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' :
